...
sql = f"""
        select
            ...
            volume
        from
            future_ohlcv_1min_bitflyer_perp
        order by
            date
        limit {analysis_width}
        """
    ...
    sql = f"""
            ...
            from
                ...
                from
                    ohlcv_1min_bitflyer_perp
                where
                    date < '{basis.iloc[0]["date"]}'
                order by
                    date desc
                limit {analysis_width - len(basis)}) as b
            order by
                date
            """
    history = repository.read_sql(database=database, sql=sql)
    basis = history.append(basis, ignore_index=True)

basis = basis.set_index("date")
rule = str(candle_min) + "min"
...
date = feature.iloc[len(feature) - 1]["date"] + datetime.timedelta(minutes=1)
open = future_open
high = future_high
low = future_low
close = future_close
